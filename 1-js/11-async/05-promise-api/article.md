...
`Promise` 클래스에는 5가지의 정적 메서드가 있습니다. 이번 챕터에선 이 메서드들의 유스 케이스에 대해서 빠르게 알아보겠습니다.

## Promise.all

여러 개의 프라미스를 동시에 실행시키고 모든 프라미스가 준비가 될 때까지 기다린다고 해봅시다.

복수의 URL에 동시에 요청을 보내고, 다운로드가 모두 완료되면 다운받은 콘텐츠를 처리할 때 이런 상황이 발생합니다.
...
배열 안 프라미스가 모두 처리되면 새로운 프라미스가 이행되고, 처리된 배열 안 프라미스의 결괏값을 담은 배열이 새로운 프라미스의 `result`가 됩니다.
...
결과 배열의 요소 순서는 `Promise.all`에 전달되는 프라미스 정렬 순서와 동일하다는 점에 유의하시기 바랍니다. `Promise.all`에 전달되는 첫 번째 프라미스가 가장 늦게 이행되더라도 처리 결과는 배열의 첫 번째 요소가 됩니다.

작업해야 할 데이터가 담긴 배열을 프라미스 배열로 매핑하고, 이 배열을 `Promise.all`로 감싸는 트릭은 자주 사용됩니다.

아래 예시에선  URL이 담긴 배열을 `fetch`를 써서 처리합니다.
...
GitHub 유저네임이 담긴 배열을 사용해 사용자 정보를 가져오는 예시를 살펴봅시다(id를 사용해 장바구니에 담긴 물건 목록을 불러올 때 동일한 로직을 사용할 수 있습니다).
...
2초 후, 두 번째 프라미스가 거부되므로 `Promise.all` 전체가 거부됩니다. 그러므로 `.catch`가 실행됩니다. 거부 에러는 `Promise.all` 전체의 결과가 됩니다.

```warn header="에러가 발생하면 다른 프라미스는 무시됩니다."
프라미스가 하나라도 거부되면 `Promise.all`은 즉시 거부되고 배열에 저장된 다른 프라미스의 결과는 완전히 잊혀집니다. 실패하지 않은 프라미스의 결과 역시 무시됩니다.

`fetch`를 사용해 여러 호출을 만들고, 그중 하나가 실패해도 호출은 계속 실행됩니다. 그러나 `Promise.all`은 다른 호출을 더 이상 신경 쓰지 않습니다. 프라미스가 처리되긴 하겠지만 그 결과는 무시됩니다.
