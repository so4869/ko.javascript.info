...
단순히 'hello word'를 출력하는 것이 아닌 복잡한 애플리케이션을 구현하려면, 내부 인터페이스와 외부 인터페이스를 구분할 수 있는 방법을 '반드시' 알고 계셔야 합니다.

잠시 개발을 벗어나 현실 세계로 눈을 돌려, 내부 인터페이스와 외부 인터페이스 구분이 무엇을 의미하는지 알아봅시다.

일상생활에서 접하게 되는 기계들은 꽤나 복잡한 구조를 가지고 있습니다. 하지만 내부인터페이스와 외부 인터페이스가 구분되어있기 때문에, 우리는 이 기계들을 문제없이 사용할 수 있습니다.
...
뭔가 디테일한 것들이 아주 많습니다. 하지만 우리는 이 모든것을 알지 못해도 커피 머신을 사용할 수 있죠.

커피 머신은 꽤나 믿음직한 기계입니다. 수 년 간 사용할 수 있죠. 중간에 고장이 나면 수리를 받으면 됩니다.

이렇게 커피 머신이 외형은 단순하지만 신뢰할만 이유는 모든 세부 요소들이 기계 내부에 잘 정리되어 *숨겨져* 있기 때문입니다.

커피 머신에서 보호 커버를 제거하면 사용법이 훨씬 복잡해지고 위험해질 겁니다(어디를 눌러야 하는지 헷갈리고 감전이 될 수도 있기 때문입니다).

커피 머신에서 보호 커버를 제거하면 어디를 눌러야 할 지 헷갈리기 때문에 사용법이 훨씬 복잡해질 겁니다. 또한 감전의 위험이 생길 수도 있습니다.

앞으로 학습 하겠지만, 프로그래밍에서 객체는 커피 머신과 같습니다. 프로그래밍에서는 안쪽 세부 사항을 숨기기 위해 보호 커버가 아닌 특별한 문법과 컨벤션이 사용된다는 점이 다릅니다.

## 내부 인터페이스와 외부 인터페이스

객체 지향 프로그래밍에서 프로퍼티와 메서드는 두 그룹으로 분류됩니다.

- *내부 인터페이스(internal interface)* -- 동일한 클래스 내의 다른 메서드에선 접근 가능하지만, 클래스 밖에선 접근할 수 없는 프로퍼티와 메서드
- *외부 인터페이스(external interface)* --  클래스 밖에서도 접근 가능한 프로퍼티와 메서드 

커피 머신으로 비유하자면, 내부 인터페이스는 기계 안쪽의 뜨거운 물이 지나가는 관이나 발열 장치 등이 될 수 있겠죠.

내부 인터페이스는 객체를 동작시키는데 사용됩니다. 각 세부 세부 요소들은 관이 발열 장치에 부착되어 동작하는 것 같이 협업하여 객체를 동작시킵니다. 

그런데 커피 머신은 보호 커버에 둘러쌓여있기 때문에 외부에서 내부로 접근할 수 없습니다. 세부 요소을 밖에선 알 수 없고, 접근도 불가능하죠. 내부 인터페이스의 기능은 외부 인터페이스를 통해야 사용할 수 있습니다.

따라서 객체를 가지고 무언가를 하기 위해선 외부 인터페이스만 알아도 됩니다. 객체 안이 어떻게 동작하는지 알지 못해도 괜찮습니다. 아주 커다란 장점이죠.

이제 어느정도 내부 인터페이스와 외부 인터페이스에 대한 감을 잡으셨을 것이라 믿습니다.

자바스크립트에는 아래와 같이 두 가지 타입의 객체 필드(프로퍼티와 메서드)가 있습니다. 

- public: 어디서든지 접근할 수 있으며 외부 인터페이스를 구성합니다. 지금까진 public 프로퍼티와 public 메서드만 다뤘습니다.
- private: 클래스 내부에서만 접근할 수 있으며 내부 인터페이스를 구성할 때 쓰입니다.

자바스크립트 이외의 다수 언어에서 클래스 자신과 자식 클래스에서만 접근을 허용하는 'protected' 필드를 지원합니다(private과 비슷하지만 상속받는 클래스에서도 접근이 가능함). protected 필드도 내부 인터페이스를 만들 때 사용됩니다. 상속 받는 클래스의 필드에 접근해아 하는 경우가 많기 때문에, protected 필드는 private 필드보다 조금 더 광범위하게 사용됩니다.

자바스크립트는 protected 필드를 지원하지 않지만, protected를 사용하면 편리한 점이 많기 때문에 이를 모방해서 사용하는 경우가 많습니다.

이제 지금까지 언급한 프로퍼티 타입을 모두를 사용해 커피머신을 만들어보겠습니다. 실제 커피 머신은 아주 복잡하지만, 여기선 간결성을 위해 간이 모델을 만들겠습니다(원한다면 구현은 가능합니다).
...
class CoffeeMachine {
  ...
  constructor(power) {
    this.power = power;
    alert( `커피 머신 객체가 만들어졌습니다.  전력량: ${power}` );
  }
...
현재 프로퍼티 `waterAmount`와 `power`는 public입니다. 따라서 `waterAmount`와 `power`를 읽고 원하는 값으로 변경하는 것은 쉽습니다.

이제 `waterAmount`를 protected로 바꿔서 통제를 강화해 보겠습니다. `waterAmount`를 0 미만의 값으로는 설정하지 못하도록 만드는 것처럼 말이죠.

**protected 프로퍼티엔 대게 프로퍼티명 앞에 밑줄 `_`이 붙습니다.**

자바스크립트에서 강제한 사항은 아니지만 밑줄은 프로그래머들 사이에서 외부 접근이 불가능한 프로퍼티나 메서드를 나타낼 때 쓰입니다.
